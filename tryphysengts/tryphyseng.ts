...
const kockaClass = function (x:number, y:number, width:number, height:number, type:string):{} {
    ...
    return {
        ...
        ...move(values),
        ...setMove(values),
        ...checkCollision(values),
        ...rigidCollision(values),
...
function checkCollision(kocka:any){
    ...
    return {
        ...
        checkCollision : ({x : x2, y : y2, width : width2, height : height2}) => {
            ...
            (kocka.y < y2 + height2) && (kocka.y + kocka.height > y2)) kocka.collision = true
...
function rigidCollision(kocka:any){
    ...
    return {
        ...
        rigidCollision : ({x : x, y : y, width : width, height : height, moveXspeed : moveXspeed, moveYspeed : moveYspeed}) => {
                ...
                if(kocka.x + kocka.width > x && kocka.x + kocka.width < x + kocka.moveXspeed+moveXspeed+1) {kocka.x = x - kocka.width;kocka.moveXspeed=0}		
                if(kocka.x < x + width && kocka.x > x + width + kocka.moveXspeed-moveXspeed-1) {kocka.x = x + width;kocka.moveXspeed=0}

        if(moveYspeed == 0 ){
            if(kocka.y + kocka.height > y && kocka.y + kocka.height < y + kocka.moveYspeed-moveYspeed+1) {kocka.y = y - kocka.height;kocka.moveYspeed=0}
            if(kocka.y < y + height && kocka.y > y + height + kocka.moveYspeed+moveYspeed-1) {kocka.y = y + height;kocka.moveYspeed=0}
        }
